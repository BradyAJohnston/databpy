...
### 1. `BlenderObject` - Highest Level (Most Convenient)
...
### 2. `AttributeArray` - Mid Level (Auto-Syncing NumPy)

`AttributeArray` is a NumPy subclass that automatically writes changes back to Blender. It provides natural NumPy operations with automatic bidirectional syncing.
...
Blender supports various attribute data types. `databpy` works with all of them:
...
Attributes exist on different **domains** of the geometry:
...
positions = db.named_attribute(obj, "position")
...
- [API Reference: Attribute](api/attribute.named_attribute.qmd)
- [API Reference: AttributeArray](api/AttributeArray.qmd)
- [API Reference: BlenderObject](api/BlenderObject.qmd)
- [API Reference: AttributeTypes](api/AttributeTypes.qmd)
- [API Reference: AttributeDomains](api/AttributeDomains.qmd)
